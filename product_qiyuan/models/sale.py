...
class SaleOrder(models.Model):
    ...
    def action_validate(self):
        ...
        if self.state == 'sale':
            ...
            if self.amount_total < outstanding_amount:
                ...
                error_str = (
                    "当前客户预付款余额为:%s,预付余额不足，请先付款!") % outstanding_amount
        else:
            error_str = "订单状态不正确!"
        if not ret:
    ...
    def _get_outstanding_info(self):
        ...
        if self.state == 'sale':
            # account_id = 4 应收账款
            # account_id = 5 预收账款
            domain = [('account_id', 'in', [4, 5]), ('partner_id', '=', self.env['res.partner']._find_accounting_partner(
                self.partner_id).id), ('reconciled', '=', False), '|', ('amount_residual', '!=', 0.0), ('amount_residual_currency', '!=', 0.0)]
...
class SaleOrderLine(models.Model):
    ...
    pieces_qty = fields.Integer(string="块数", compute='_compute_pieces_qty',store=True)
